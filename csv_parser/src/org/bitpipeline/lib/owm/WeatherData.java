...
public class WeatherData extends AbstractWeatherData {
	...
	public static class Wind extends AbstractWeatherData.Wind {
		...
		public Wind (JSONObject json) {
			this.speed = (float) json.optDouble (JSON_SPEED);
			this.deg = json.optInt (JSON_DEG, Integer.MIN_VALUE);
			this.gust = (float) json.optDouble (JSON_GUST);
			this.varBeg = json.optInt (JSON_VAR_BEG, Integer.MIN_VALUE);
			this.varEnd = json.optInt (JSON_VAR_END, Integer.MIN_VALUE);
		}
