...
public class WeatherHistoryCityResponse extends AbstractOwmResponse {
	...
	public WeatherHistoryCityResponse (JSONObject json) {
		...
		String calcTimeStr = json.optString (JSON_CALCTIME);
		this.calctimeFind = getValueFromCalcTimeStr(calcTimeStr, WeatherHistoryCityResponse.JSON_CALCTIME_FIND);
		this.calctimeFetch = getValueFromCalcTimeStr(calcTimeStr, WeatherHistoryCityResponse.JSON_CALCTIME_FETCH);
		this.cityId = json.optInt (WeatherHistoryCityResponse.JSON_CITY_ID, Integer.MIN_VALUE);
		
		JSONArray jsonHistory = json.optJSONArray (JSON_LIST);
		if (jsonHistory == null) {
